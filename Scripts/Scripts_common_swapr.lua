--- ===================================== SWAPR CONFIG =====================================
env.info("SWAPR Run")
  local SWAPR_Prefixes = {"Minhad_", "Starfighter_", "Nellis_", "Moose_", "_Nickel", "Ramp_", "Shooter_", "_AB"}  -- Add all the relevant client prefixes / suffixes to this table
  ...
  local Replacement_Type = "Static" -- Must be set to either "AI" or "Static". If set to "AI", any targeted clients will receive uncontrolled AI replacements. If set to "Static", clients located on open parking spots (or within airbase areas) or on open terrain (far from FARPs and airbases) will receive static replacements (the rest of targeted clients will receive AI replacements instead, to work around DCS bugs)
  
  local Hidden_AI_Replacements = false -- Must be set to either true or false. If true, generated AI replacements will be hidden on F10 map (doesn't work for static replacements and doesn't work in MP!) 
