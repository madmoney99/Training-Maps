...
function RangeCau1:OnAfterImpact(From, Event, To, result, player)  
...
function RangeCau2:OnAfterImpact(From, Event, To, result, player)  
   result.messageType = 4 
   result.callsign = player.playername
   result.theatre = env.mission.theatre
   result.rangeName = self.rangename
...
function AAWR:OnAfterImpact(From, Event, To, result, player)  
   result.messageType = 4
   result.callsign = player.playername
   result.theatre = env.mission.theatre
   result.rangeName = self.rangename
...
local shellNorth=AUFTRAG:NewTANKER(zoneTankerNorth:GetCoordinate(), drogueAltitude, UTILS.KnotsToAltKIAS(tankerSpeed,drogueAltitude), 180, tankerTrackLength)
...
local texNorth=AUFTRAG:NewTANKER(zoneTankerNorth:GetCoordinate(), boomAltitude, UTILS.KnotsToAltKIAS(tankerSpeed,boomAltitude), 180, tankerTrackLength)
...
local shellSouth=AUFTRAG:NewTANKER(zoneTankerSouth:GetCoordinate(), drogueAltitude, UTILS.KnotsToAltKIAS(tankerSpeed,drogueAltitude), 125, tankerTrackLength)
...
local texSouth=AUFTRAG:NewTANKER(zoneTankerSouth:GetCoordinate(), boomAltitude, UTILS.KnotsToAltKIAS(tankerSpeed,boomAltitude), 125, tankerTrackLength)
...
local texLow=AUFTRAG:NewTANKER(zoneTankerLow:GetCoordinate(), 12000, UTILS.KnotsToAltKIAS(250,12000), 125, tankerTrackLength)
